...
export class BlockingContext {
    private preloadScriptId: string | undefined
    ...
    constructor(
        ...
        this.onIsMutationObserverEnabled = (event) =>
            blocker.onIsMutationObserverEnabled(event)
    }
    ...
    public disable(): void {
        ...
        if (this.blocker.config.loadCosmeticFilters === true) {
            if (this.preloadScriptId !== undefined) {
                this.session.unregisterPreloadScript(this.preloadScriptId)
                this.preloadScriptId = undefined
            }
...
export class ElectronBlocker extends FiltersEngine {
    private readonly contexts: WeakMap<Electron.Session, BlockingContext> =
        new WeakMap()
    ...
    public onHeadersReceived = (
        ...
        if (
            ...
            const rawCSP: string | undefined = this.getCSPDirectives(
    ...
    public onBeforeRequest = (
        ...
        if (
            this.config.guessRequestTypeFromUrl === true &&
            request.type === 'other'
        ) {
